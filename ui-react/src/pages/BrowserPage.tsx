  ...
  return (
    <div className="arc-page-scroll mx-auto h-full max-w-5xl overflow-y-auto px-4 py-8 pb-24 sm:px-8 lg:px-12">
      ...
      <div className="mb-8 grid gap-3 sm:grid-cols-2">
        <button onClick={() => go("https://arena.ai", "arena")} className="arc-card flex items-center gap-3 rounded-2xl p-4 text-left hover:border-[#6366F1]/40 active:scale-[.99]">
        ...
        <button onClick={() => go("https://mail.google.com", "webmail")} className="arc-card flex items-center gap-3 rounded-2xl p-4 text-left hover:border-[#6366F1]/40 active:scale-[.99]">
      ...
      <div className="arc-card rounded-3xl p-5">
        <p className="arc-mono mb-3 text-[10px] uppercase tracking-[.22em] text-[#6366F1]/70">Open any site</p>
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          <button onClick={() => url.trim() && go(/^https?:\/\//.test(url.trim()) ? url.trim() : `https://${url.trim()}`, "free")}
            className="flex items-center justify-center gap-2 rounded-xl bg-[#6366F1] px-5 py-2.5 text-xs font-bold text-white active:scale-[.98]">
            <Zap size={14} />Launch
          </button>
        </div>
      </div>
